...
namespace TsGui.Queries
{
    public class CompareQuery: BaseQuery, ILinkTarget
    {
        ...
        public override async Task<ResultWrangler> ProcessQueryAsync(Message message)
        {
            ...
            string first = (await this._querylist.Queries[0]?.GetResultWranglerAsync(message))?.GetString();

            for (int i = 1; i < this._querylist.Queries.Count; i++)
            {
                string second = (await this._querylist.Queries[i].GetResultWranglerAsync(message))?.GetString();
                wrangler.NewResult();
