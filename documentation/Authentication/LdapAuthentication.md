...
LDAP authentication provides authentication against a domain for use with LDAP queries. This makes use of the Novell.Directory.Ldap library rather than the Microsoft libraries which are missing from WinPE. 
...
* [LDAP Queries](#ldap-queries)
  * [LdapGroupMembers Query](#ldapgroupmembers-query)
    * [LdapGroupMembers query example](#ldapgroupmembers-query-example)
  * [LdapOU Query](#ldapou-query)
    * [LdapOU query example](#ldapou-query-example)
...
## Overview
LDAP Authentication requires three things in your TsGui configuration:
...
```xml
<Authentication Type="LDAP" AuthID="ldap_auth" Domain="domain.local"/>
```
...
You can accept authentication based on the groups the user is a member of the using one of the following formats:
...
```xml
<Authentication Type="LDAP" AuthID="ldap_auth" Domain="domain.local" Groups="group 1, group 2" />
```

<ins>As elements</ins><br>
Add a **Groups** element, then add a \<Group> element for each group
```xml
<Authentication Type="LDAP" AuthID="ldap_auth" Domain="domain.local">
  <Groups>
    <Group>group 1</Group>
...
```xml
<Authentication Type="LDAP" AuthID="ldap_auth" Domain="domain.local" Groups="group 1, group 2" RequireAllGroups="TRUE" />
```

#### Using group membership to change the UI
It is sometimes useful to change the UI based on who is logged in. By setting the **CreateGroupIDs** attribute to TRUE, TsGui will create a variable with a matching ID for each group that can be used with the [option linking feature](/documentation/features/OptionLinking.md). The ID created for each group will in the format **%AuthID%_%GroupName%**\*. 
...
```xml
	<Authentication Type="LDAP" AuthID="ldap_auth" Domain="domain.local" Groups="group 1,group 2" CreateGroupIDs="TRUE" />

	<Page>
		<Row>
			<Column>
				<GuiOption Type="UsernameBox" AuthID="ldap_auth">
					<Variable>VAR_Username</Variable>
				</GuiOption>

				<GuiOption Type="PasswordBox" AuthID="ldap_auth" />

				<GuiOption Type="ActionButton" IsDefault="TRUE">
					<Action Type="Authentication" AuthID="ldap_auth" />
				...
				<GuiOption Type="CheckBox">
					...
					<Label>group 1</Label>
					<SetValue>
						<Query Type="LinkTo">ldap_auth_group1</Query>
					</SetValue>
				</GuiOption>
				...
				<GuiOption Type="CheckBox">
					...
					<Label>group 2</Label>
					<SetValue>
						<Query Type="LinkTo">ldap_auth_group2</Query>
					</SetValue>
				</GuiOption>
...
**Note:** usernames are only supported in UPN format at this time e.g. **username@domain.local**. 
...
### Login Example
The following example shows the configuration items above in position in a TsGui config file. Note the **ldap_auth** value across the configuration pieces. 

```xml
<TsGui LiveData="TRUE">
d	<Authentication Type="LDAP" AuthID="ldap_auth" Domain="domain.local"/>
...
## LDAP Queries
In order to query LDAP, you must first authenticate. Once the user has been successfully authenticated using the options above, you can then use that session to run queries against the directory. TsGui currently supports the following LDAP queries:

* LdapOU - Queries an OU structure. This is ideal for use with a TreeView GuiOption
* LdapGroupMembers - Queries the members of a group. This is ideal for use with a DropDownList GuiOption

All LDAP queries require the **AuthID** attribute. This will run the query using the autheticated session created above.

---
<br>


### LdapGroupMembers Query
...
#### LdapGroupMembers query example
```xml
<Query Type="LdapGroupMembers" AuthID="ldap_auth">
...
#### LdapOU query example
```xml
<Query Type="LdapOU" AuthID="ldap_auth">
...
Note the **ldap_auth** value across the configuration pieces. 

```xml
<GuiOption Type="DropDownList">
    <Variable>LDAP_User</Variable>
